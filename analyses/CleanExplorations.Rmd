...
# We train the logistic regression model using GAM
lr1gam <- gam(GroundTruth ~ factor(Activity) + factor(Social) + factor(Focus) + s(PCALoad1), family=binomial(link='logit'), data=behdata)
summary(lr1gam)
anova(lr1gam, test="Chisq") # PCA Load is a very significant predictor
1 - (lr1gam$deviance / lr1gam$null.deviance) # Pseudo-rsquared
...
lr2gam <- gam(GroundTruth ~ factor(Activity) + factor(Social) + factor(Focus) + s(PCALoad1), family=binomial(link='logit'), data=behdata2) # The PCA load coefficient goes in the wrong direction!!!!???
summary(lr2gam)
anova(lr2gam, test="Chisq") # PCA Load is a very significant predictor
1 - (lr2gam$deviance / lr2gam$null.deviance) # Pseudo-rsquared
...
lr3gam <- gam(GroundTruth ~ factor(Activity) + factor(Social) + factor(Focus) + s(PCALoad1), family=binomial(link='logit'), data=behdata3) # The PCA load coefficient goes in the wrong direction!!!!???
summary(lr3gam)
anova(lr3gam, test="Chisq") # PCA Load is a very significant predictor
1 - (lr3gam$deviance / lr3gam$null.deviance) # Pseudo-rsquared
...
lr4gam <- gam(GroundTruth ~ factor(Activity) + factor(Social) + factor(Focus) + s(PCALoad1), family=binomial(link='logit'), data=behdata4) # The PCA load coefficient goes in the wrong direction!!!!???
summary(lr4gam)
anova(lr4gam, test="Chisq") # PCA Load is a very significant predictor
1 - (lr4gam$deviance / lr4gam$null.deviance) # Pseudo-rsquared
...
lr4bgam <- gam(GroundTruth ~ factor(Activity) + factor(Social) + factor(Focus) + s(PCALoad1), family=binomial(link='logit'), data=behdata4b) # The PCA load coefficient goes in the wrong direction!!!!???
summary(lr4bgam)
anova(lr4bgam, test="Chisq") # PCA Load is a very significant predictor
1 - (lr4bgam$deviance / lr4bgam$null.deviance) # Pseudo-rsquared
...
summary(lm4)
anova(lm4)
# Some trends/differences among with/without assistance
lm4ass <-lm(PCALoad1 ~ Activity + Social + Focus, data=behdata4[behdata4$GroundTruth==0,])
summary(lm4ass)
anova(lm4ass)
lm4noass <-lm(PCALoad1 ~ Activity + Social + Focus, data=behdata4[behdata4$GroundTruth==1,])
summary(lm4noass)
anova(lm4noass)
# With only extreme episodes --- Better predictive power
lm4b <- lm(PCALoad1 ~ Activity + Social + Focus, data=behdata4b)
summary(lm4b)
# Some trends/differences among with/without assistance
lm4bass <-lm(PCALoad1 ~ Activity + Social + Focus, data=behdata4b[behdata4b$GroundTruth==0,])
summary(lm4bass)
anova(lm4bass)
lm4bnoass <-lm(PCALoad1 ~ Activity + Social + Focus, data=behdata4b[behdata4b$GroundTruth==1,])
summary(lm4bnoass)
anova(lm4bnoass)

```

TODO: maybe at the end try several lm models, contrasting expert teachers vs. novice teachers, tabletop tech vs. laptop tech...?