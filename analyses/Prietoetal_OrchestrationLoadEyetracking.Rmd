...
* Datasets for Study 2 (secondary school, expert teacher, either using [usual laptop-based technology](https://zenodo.org/record/16551/) or novel technology (URL TO BE ADDED))
* Dataset for Study 3 (Multi-tabletop sessions in the lab, primary school students, with/without a helper) (URL TO BE ADDED)
...
TODO: Add the links and preprocessing code for the studies, once all the raw datasets are in a public repository (for now, the preprocessing code is the beginning of the separate StudyX.Rmd files).

The current script assumes that you will put the processed datafiles in ```./data/study1```, ```./data/study2```, etc

```{r, message=FALSE, warning=FALSE, echo=FALSE}
source("./lib/outliers.R")
source("./lib/loadIndex.R")

# Load preprocessed data (remove unused variables, aggregate in 10s episodes, align and merge eyetracking and videocoded data)
# Study numbering in this block follows the old (chronological) numbering! (study 1 is called study 4 in the manuscript, study 2 is called study 1 in the manuscript, etc.)
# Data for study 1
...
# The "basic load index" is used for the sampling of 'agreement episodes' to be coded (we video code episodes with load index of 0 or 4)
loaddata <- calculateLoadIndexAndNormalizations(totaldata,3:6,stablenorm = 3)
# TODO: Do we still need these indexes? maybe this can be removed
...
## Overall dataset: Calculate OLS (1st PCA score)
...
## Study 2 (Study 1 in the manuscript order)
```{r, message=FALSE, warning=FALSE}
...
1 - (lr2$deviance / lr2$null.deviance) # Pseudo-rsquared=0.88
...
lr3 <- glm(GroundTruth ~ Activity + Social + Focus + PCALoad1, family=binomial(link='logit'), data=behdata3) # OLS is a significant predictor
summary(lr3)
anova(lr3, test="Chisq") # OLS is a significant predictor
...
lr4b <- glm(GroundTruth ~ Activity + Social + Focus + PCALoad1, family=binomial(link='logit'), data=behdata4b) # OLS coefficient goes in the right direction, but is not significant
summary(lr4b)
anova(lr4b, test="Chisq") # OLS is not marginally significant predictor
...
### Study 2 (Study 1 in the new numbering) Data
...
### Study 3 (Study 2 in the new numbering) Data
...
### Study 4 (Study 3 in the new numbering) Data
...
### Study 1 (Study 4 in the new numbering) Data
...
### Laptops vs. Tabletops classrooms
