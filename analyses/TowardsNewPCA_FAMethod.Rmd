...
d4$PCALoad = pca.study4$ind$coord[,1]
```
...
lr1 <- glm(TeacherLoad ~ value.Activity + value.Social + value.Focus + PCALoad, family=binomial(link='logit'), data=behdata) # The PCA load coefficient goes in the wrong direction!!!!???
summary(lr1)
...
# Same thing with the raw (instead of normalized) values

```{r, message=FALSE, warning=FALSE}

# We select only the interesting columns
d4b <- loaddata[,c(1,2,3:6,7:10)]

# To make the target variable (value.Experimental) clearer (we want our measure to distinguish C1 from C2, ignoring COM samples), we put our "ground truth" in a new variable (1 if high load, 0 if low load, NA otherwise)
d4b$TeacherLoad <- ifelse(d4b$value.Experimental=="C2",1,ifelse(d4b$value.Experimental=="C1",0,NA))
d4b <- d4b[,-7]
...
# We calculate the PCA load index (score in the 1st PCA component)
library(FactoMineR)
pca.study4 = PCA(d4b[, 3:6], scale.unit=TRUE, ncp=2, graph=F)
plot.PCA(pca.study4, axes=c(1, 2), choix="var", title="PCA All sessions, Study 4")
# We add the PCA load index to our dataset
d4b$PCALoad = pca.study4$ind$coord[,1]
```
...
```{r, message=FALSE, warning=FALSE}
# clean up further the dataset, to keep only those samples for which we have process variables
behdata <- d4b[!is.na(d4b$value.Activity) & !is.na(d4b$value.Social) & !is.na(d4b$value.Focus),]
# We take only the samples for which we have a meaningful ground truth
behdata <- behdata[!is.na(behdata$TeacherLoad),]
# ... and we remove the samples with categorical variables that appear only very seldom
behdata <- behdata[behdata$value.Activity!="EXP",]
...
# We train the logistic regression model
lr1 <- glm(TeacherLoad ~ value.Activity + value.Social + value.Focus + PCALoad, family=binomial(link='logit'), data=behdata) # The PCA load coefficient goes in the right direction, but is not significant
summary(lr1)
anova(lr1, test="Chisq") # PCA Load is not a significant predictor
```
...
# We train the logistic regression model
lr2 <- glm(TeacherLoad ~ value.Activity + value.Social + value.Focus + FALoad, family=binomial(link='logit'), data=behdata) # The FA load coefficient goes in the right direction, but is not significant
summary(lr2)
anova(lr2, test="Chisq") # FA Load is not a significant predictor
