...
\item An analysis of variance of the linear model shows that the social plane of interaction, the focus of the gaze and the session number were significant predictors in this model of OLS ($F=157; p-value<0.001$, $F=45; p-value<0.001$ and $F=22; p-value<0.001$), while the teaching activity itself was not ($F=2.59; p-value=0.05$)
\item As we can see in Table \ref{tab:case4results}, the main significant trends are that the class-level of interaction was higher load than the small-group level; that looking at student faces or the teacher desk full of manipulatives (clutter!) was higher than looking at the tabletop itself; and that sessions 2 and 3 were higher load than session 1
\item The reasons behind these trends cannot be directly deduced from the model, and need qualitative analysis of the video to develop conjectures
\end{itemize}
...
\hline
\hline
Intercept (\textbf{Explanation, Class-level, Focus on student backs, Session 1}) & \textbf{-0.39} & 0.11 & -3.74 & \textbf{$<$0.001***}\\
Activity: Monitoring & -0.01 & 0.07 & -0.14 & 0.89\\
Activity: Repairs & -0.011 & 0.07 & -0.15 & 0.88\\
Activity: Task distribution & -0.06 & 0.08 & -0.79 & 0.43\\
\textbf{Social: Small group} & \textbf{-0.29} & 0.12 & -2.44 & \textbf{0.02*}\\
\textbf{Focus: Student faces} & \textbf{0.38} & 0.13 & 2.90 & \textbf{0.004**}\\
\textbf{Focus: Teacher desk} & \textbf{0.44} & 0.16 & 2.70 & \textbf{0.007**}\\
\textbf{Focus: Tabletop computer} & \textbf{-0.52} & 0.14 & -3.68 & \textbf{$<$0.001***}\\
...
\item These results (sp. case 3) also show some of the limitations of the method -- a certain volume of data is needed for OLS to discriminate the load of different situations (e.g., several sessions, not just small parts of one)
...
\item Are the situations really authentic (observers, ETs)? subjects did not complain/distract, and eyetrackers are becoming more discrete
...
\item Applying the same kind of modelling to the data of all our studies, we can begin to get insights into the kind of influence that these process variables have on orchestration load (Table \ref{tab:trends}, the complete data analysis and results is available online\footnote{TODO: Add github repo}) 


... (some common, some teacher- or techsetup- specific) ...

\item Additional aspects are noteworthy... focus on technology does not seem to be especially high load ... technology affecting the mix of activities and hence the OL, without technology being the focus per se -- one reason we need to measure this in authentic settings, not just in the lab (where focus is often forced on the technology)
\end{itemize}
...
% if using array.sty, it might be a good idea to tweak the value of
% \extrarowheight as needed to properly center the text within the cells
\caption{Summary of the process variable factors that, and indications about their effect sizes (in terms of Cohen's d): '+/-' for small effects ($d<0.3$); '++/--' for medium effects ($0.3<d<0.7$); '+++/- - -' for large effects ($d>0.7$)}
\label{tab:trends}
\centering
%% Some packages, such as MDW tools, offer better commands for making tables
%% than the plain LaTeX2e tabular which is used here.
...
\item The statistical models built accepting OLS as a reliable measure of load, provide a first glimpse of how different activities, , influence the load of teachers in different situations
