...
\item This approach to classroom usability reminds of the beginnings of usability studies \cite{Webusability} (focus on a few users finishing the task -- cf. notion of ``teacher heroes'' \cite{Dillenbourg2009b})
...
\end{multline}
\item In this model, as designers of technology, we are interested mostly on measuring the correction factor on orchestration load due to our technologies (e.g., $\beta_T*T$), once we take away the effect of the known process variables (A, T, S), keeping the rest of the variables in the classroom situation as constant as possible.
\end{itemize}
