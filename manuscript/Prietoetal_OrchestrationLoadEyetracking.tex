...
In the data of this study, once we take away the influence of the process variables, the OLS shows positive correlation (i.e., coefficient) with the ground truth. When building a logistic regression model using the `agreement episodes' as in previous studies ($n=163$ episodes), the OLS was no longer a significant predictor of the binary orchestration load we assumed as ground truth ($p=0.13$). In order to test the conjecture that this lack of significance was tied to the low volume of data used to build the model, we videocoded the full extent of the sessions in this study. As we can see in Table \ref{tab:case3results}, the new model built with additional videocoded data ($n=610$ episodes) finds the \textit{OLS as a significant predictor} of the assumed ground truth ($p=0.016$). In both cases, however, the logistic regression models explain relatively little variance in the data (McFadden's pseudo-$R^2=0.07$, in the model with more samples). 

The above modelling results provide additional supporting evidence for the OLS as a discriminant measure of orchestration load, but also highlight some of the limitations of the proposed method in terms of the amount of data needed to build the statistical models of orchestration load. Especially in cases like this one, where we look at data representing only \textit{a part} of a session (vs. stretches of multiple sessions, as in previous studies), a coding of the full extent of the session (vs. only the `agreement episodes') might be in order, so as to gather enough information to build the models.
...
Another important limitation of the method presented here is the fact that it is still quite expensive (both in terms of equipment and human effort, since they require researcher manual work to code the process variables from the eye-tracking videos). However, as eye-tracking technology becomes cheaper % (together with the fact that the measures used do not require high accuracy in terms of determining exactly where the teachers are looking), 
